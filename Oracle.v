...
Proof.
  ...
  unfold Rpower.
  unfold Rge.
  specialize (B_user_w_nonneg_hyp u st) as H_.
  unfold Rge in H_.
...
Proof.
  ...
  assert (Heq38 : forall t, (fun t => Lambda_x u t st) t = eq38 t).
  {
    ...
    - apply Users_NoDup.
    - intro. Search Rge Rle. apply Rle_ge. apply W_user_nonneg_hyp.
    - admit.
  }
...
Hypothesis Q_nonneg_hyp : forall st, 0 <= Q st.
...
Proof.
  intros.
  unfold Q'.
  pose proof (decay_pos (t_sub st - tu u st)).
  pose proof (decay_pos (t  - t_sub st)).
  pose proof (W_user_nonneg_hyp u st).
  pose proof (Q_nonneg_hyp st).
  unfold w.
  unfold wu.
  pose proof (L_f_nonneg_hyp u st).
  try lra.
  Admitted.

Theorem balance_after_submission_reward_positive :
  forall t' St u,
    (B_oracle_r St) - (reward_payout u t' St) > 0.
Proof.
  intros.
  unfold reward_payout.
  replace (B_oracle_r St - alpha * B_oracle_r St * (w u (V St) / Q' u t' (V St)) * (1 - decay (t' - tu u (V St)))) with
    ((B_oracle_r St) * (1 - alpha * (w u (V St) / Q' u t' (V St)) * (1 - decay (t' - tu u (V St))))).
  2 : {  field. admit. }
  Admitted.
